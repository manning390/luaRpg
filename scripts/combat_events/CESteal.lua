...
function CESteal:StealFrom(target)
	local success = Formula.Steal(self.mState, self.mOwner, target)
	local entity = self.mState.mActorCharMap[target].mEntity

	local x = entity.mX
	local y = entity.mY
	local effect = AnimEntityFx:Create(x, y, self.mAnim, self.mAnim.frames)
