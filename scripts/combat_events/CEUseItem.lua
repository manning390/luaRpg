...
function CEUseItem:DoUseItem()
	...
	local effect = AnimEntityFx:Create(pos:X(), pos:Y(), gEntities.fx_use_item, gEntities.fx_use_item.frames, 0.1)
	self.mState:AddEffect(effect)

	local action = self.mItemDef.use.action
	CombatActions[action](self.mState, self.mOwner, self.mTargets, self.mItemDef)
end
